...
class RootActor[P](
  ...
  def gathering(state: State[P]): Receive = {
    ...
    case scored: ScoredParticles[P] =>
      context.become(gathering(logic.addNewLocallyScoredParticles(state, scored, sender)))
    case weighted: WeighedParticles[P] =>
      val newState = logic.addNewWeighted(state, weighted, sender)
      if(newState.dueToFlush) context.become(waitingForFlushComplete(newState))
    case MixNow =>
      logic.doMixing(state)
