...
//    }
    ...
    "counts of booleans as a histogram" in {
      val emp = IndexedSeq(true,true,false,true).toEmpiricalTable
      
      val name = "Booleans"
      
      grapher.writeBooleanHistogram(name, emp)
      
      val writtenScript = Source.fromFile(new File(homePath.resolve("scriptBoolean").toString)).mkString
      
      val expectedScript =
"""setwd("/home/user/workspace/Sampler/examples/r")
require(ggplot2)
data <- read.csv("Booleans.csv")
