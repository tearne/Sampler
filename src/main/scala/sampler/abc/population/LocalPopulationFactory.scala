...
class LocalPopulationFactory(runner: LocalJobRunner) extends PopulationFactory{
	def run(model: ABCModel)(pop: Empirical[model.Parameters], jobSizes: Seq[Int], tolerance: Double): Seq[Try[model.Population]] = {
		...
		Population(model)(pop, quantity, tolerance, aborter)
		}}
		
		runner.apply(jobs)
	}
}
object LocalPopulationFactory{
	def apply() = new LocalPopulationFactory(ParallelCollectionRunner())
}