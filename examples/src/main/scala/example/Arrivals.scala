...
object Arrivals extends App{
	...
	val flightArrivalsTimes = timetable.combine(wind)((time, wind) => 
		(time.inSeconds + wind * 60).toInt.seconds
	)
	...
	val immigrationHallArrivals = new Samplable[IndexedSeq[Int]]{
		def sample() = {
			...
			val arrivalHallTimes = 
				...
				.groupBy(identity)
				.mapValues(_.size)
			val arrivalsBySecond = 
				(1 to Time.dayInSeconds)
				.map(i => arrivalHallTimes.getOrElse(i.seconds, 0))
			arrivalsBySecond
		}
	}	
...
//	val arrivals = immigrationHallArrivals.sample
	...
	val processingTime = {
		val quick = normalTime(15.seconds, 2.seconds)
...
object ArrivalsUtils{
	...
	def normalTime(mean: Time, vari: Time) = Samplable
		.normal(mean.inSeconds, vari.inSeconds)
		.map(_.toInt.seconds)
