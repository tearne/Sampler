...
object FlockMortality extends App {
  ...
  val days = observed.dailyDead.indices
...
object FlockMortalityModel extends Model[FlockMortalityParams] {
  ...
  val flockSize: Int = 3000
  val eggCoef: Double = 2200.0 / 3000.0

  val prior: Prior[FlockMortalityParams] = new Prior[FlockMortalityParams] {
    def density(p: FlockMortalityParams): Double = {
    ...
    val distribution: Distribution[FlockMortalityParams] = Distribution.from(r =>
  ...
  private val kernel: Prior[Double] = new Prior[Double] {
    /*
  ...
  val threeDie: Distribution[Int] = Distribution.uniform(IndexedSeq(-1, 0, 1))

  private def threeDensity(v: Int) = if (v <= 1 || v >= -1) 1.0 / 3 else 0.0

  def perturb(p: FlockMortalityParams): FlockMortalityParams = {
    import p._
  ...
  def perturbDensity(a: FlockMortalityParams, b: FlockMortalityParams): Double =
  ...
  object Parameters {
    val header: Seq[String] = Seq("Beta", "Eta", "Gamma", "Delta", "Sigma", "Sigma2", "Offset")
  }

  def distanceToObservations(p: FlockMortalityParams): Distribution[Double] = {
    val t: Distribution[Simulated] = modelDistribution(p)
    t.map(sim => sim.distanceToObserved)
    t.map(_.distanceToObserved)
  }
  ...
  case class Simulated(dayStates: Map[Int, ODEState]) {
    def distanceToObserved: Double = {
      val accumulatedSimDead = observed.dailyDead.indices.map(dayStates(_).d.toInt) //No Sum since already accumulated when simulated
      val accumulatedObsDead = observed.dailyDead.scanLeft(0) { case (a, v) => a + v.toInt }.tail

      //      val accumulatedSimEggs = observed.dailyEggs.indices.map(i => dayStates(i).eggs).scanLeft(0) { case (a, v) => a + v.toInt }.tail
      ...
      val newObsDead = accumulatedObsDead map (_.toDouble / deadMax)
      val newSimDead = accumulatedSimDead map (_.toDouble / deadMax)

      val newObsEggs = observed.dailyEggs map (_.toDouble / eggsMax)
      val newSimEggs = observed.dailyEggs.indices.map(dayStates(_).eggs) map (_ / eggsMax)

      val error = newObsDead.indices.foldLeft(0.0) { case (acc, dayIndex) =>
  ...
  def modelDistribution(p: FlockMortalityParams): Distribution[Simulated] = {
    val days = observed.dailyDead.indices
      ...
      val shiftedResultsMap =
        if (p.offset == 0) unshiftedResultsMap
          ...
          val additions = (0 until p.offset).foldLeft(Map[Int, ODEState]()) { case (map, day) =>
            map.updated(day, ODEState(flockSize, 0, 0, 0, 0, observed.dailyEggs.head))
          }
          tmp.++(additions)
        }
